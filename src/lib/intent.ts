...
9. You must NOT generate bounding boxes or coordinates.
   Spatial geometry comes only from computer-vision
   models (Grounding DINO / SAM2) in a separate pipeline.
...
---------------------------------------------------------
REGIONS — DO NOT OUTPUT
---------------------------------------------------------

Do NOT output any "regions" array and do NOT invent
bounding boxes, rectangles, or pixel/normalized
coordinates of any kind.

Spatial geometry (bounding boxes and masks) is produced
exclusively by computer-vision models (Grounding DINO,
SAM2, land-cover segmentation) in a separate pipeline.
Any coordinates you generate cannot be verified and will
be discarded.

If the user asks "where" something is, describe the
location in words (e.g. "the western coastline") inside
your answer text or via the "region" label of an object.
`;
